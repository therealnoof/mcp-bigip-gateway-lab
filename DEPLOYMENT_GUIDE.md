...
Build order:
1. JWT Key (Step 1 in APM guide)
2. OAuth Scope (Step 2)
3. OAuth AS Profile (Step 3)
4. Client Application registration (Step 4) — note the auto-generated Client ID and Secret
5. Local User DB + user for ROPC (Step 5)
6. OAuth AS VIP + access policy — test token issuance (Step 6)
7. MCP Gateway VIP + per-request policy (Step 7)
8. End-to-end test (Step 8)

> **Important:** BIG-IP APM does not support the `client_credentials` OAuth grant
> type. This lab uses Resource Owner Password Credentials (ROPC) as a workaround.
> See the APM Configuration Guide for details and production recommendations.
...
agent:
  environment:
    ...
    - BIGIP_CLIENT_ID=<your auto-generated client ID>
    - BIGIP_CLIENT_SECRET=<your auto-generated client secret>
...
> **Note:** The Client ID and Client Secret are auto-generated by BIG-IP when
> you create the client application. They will not match any pre-planned values.
> The agent must also send `username`, `password`, and `grant_type=password`
> because this lab uses the ROPC flow (see APM Configuration Guide for why).
...
echo ""
echo "=== 10. BIG-IP OAuth Token (ROPC) ==="
curl -sk -X POST https://10.1.10.110/f5-oauth2/v1/token \
  -d 'grant_type=password&client_id=YOUR_CLIENT_ID&client_secret=YOUR_CLIENT_SECRET&username=mcp-agent&password=AgentPass2024!&scope=mcp:tools' | jq .token_type
```
...
| 10. OAuth token | `"Bearer"` | Check APM OAuth profile, client app, LocalDB user, and access policy |
